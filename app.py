...
if uploaded_file:
    ...
    for _, r in data.iterrows():
        ...
        folium.CircleMarker(location=[r["Latitude"], r["Longitude"]],
                            radius=5, fill=True, popup=popup).add_to(mc)
    ...
    if st.button("Plot Section"):
        ...
        fig.update_layout(title=f"Section: {start} → {end} (≈{seg_len:.0f} m)",
                          xaxis_title="Chainage (m)", yaxis_title="Elevation", template="plotly_white")
